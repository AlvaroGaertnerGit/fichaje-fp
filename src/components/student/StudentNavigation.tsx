...
const links = [
  { href: "/", label: "Fichaje" },
  { href: "/historial", label: "Archivo" },
  { href: "/cuenta", label: "Cuenta" },
] as const;

export function StudentNavigation({ studentName }: { studentName: string }) {
    ...
    <header className="border-b border-ink">
      <div className="mx-auto flex max-w-3xl items-center gap-8 px-6 py-4">
        ...
        <div className="ml-auto flex items-center gap-4">
          <span className="hidden font-mono text-xs text-ink-faint sm:inline">{studentName}</span>
          <form action={logout}>
            <button
              type="submit"
